...
- **Theme edits need a full render.** After changing `theme/*.scss` run `quarto render` (whole project), not `quarto render <one-file>` — a single-file render reuses the cached compiled theme bundle. If a theme change still doesn't show, delete `.quarto/` and re-render.
